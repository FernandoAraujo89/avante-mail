// Operações imutáveis sobre o documento do Criador de email.

import type { Block, Column, EmailDesign, Row } from "./types";
...
export function updateRowAttrs(
  ...
  return replaceRows(
    design,
    design.rows.map((r) =>
      r.id === rowId ? { ...r, attrs: { ...r.attrs, ...patch } } : r
    )
  );
}
...
export function moveBlockTo(
  ...
  if (!location) return design;

  let index = targetIndex;
  if (
    location.row.id === targetRowId &&
    location.column.id === targetColId
  ) {
    const originalIndex = location.column.blocks.findIndex(
      (b) => b.id === blockId
    );
