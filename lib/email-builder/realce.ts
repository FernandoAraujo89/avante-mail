...
function realcarCss(css: string): Token[] {
  ...
  while (i < css.length) {
    ...
    if (css[i] === "{" || css[i] === "}") {
      dentroDeBloco = css[i] === "{";
      saida.push({ tipo: "pontuacao", texto: css[i] });
      i += 1;
      continue;
    }
    ...
    if (proxima === i) {
      saida.push({ tipo: "texto", texto: css[i] });
      i += 1;
      continue;
    }
...
export function realcarHtml(codigo: string): Token[] {
  ...
  while (i < codigo.length) {
    ...
    if (abre && (NOME.test(proximo ?? "") || proximo === "/")) {
      ...
      let nome = "";
      while (i < codigo.length && NOME.test(codigo[i])) {
        nome += codigo[i];
        i += 1;
      }
      empurrar("tag", nome);
      ...
      while (i < codigo.length && codigo[i] !== ">") {
        if (/\s/.test(codigo[i])) {
          let espaco = "";
          while (i < codigo.length && /\s/.test(codigo[i])) {
            espaco += codigo[i];
            i += 1;
          }
